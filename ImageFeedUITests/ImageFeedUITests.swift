...
class ImageFeedUITests: XCTestCase {
    ...
    func testProfile() throws {
        ...
        let nameLabel = app.staticTexts["Dionisii Konevichenko"]
        XCTAssertTrue(waitForElement(nameLabel, timeout: 10), "Метка имени не найдена")
        XCTAssertEqual(nameLabel.label, "Dionisii Konevichenko", "Текст метки имени не соответствует ожидаемому")
        
        // Проверяем наличие и текст метки логина
        let loginLabel = app.staticTexts["@BrotherZemurro3"]
        XCTAssertTrue(waitForElement(loginLabel, timeout: 10), "Метка логина не найдена")
        XCTAssertEqual(loginLabel.label, "@BrotherZemurro3", "Текст метки логина не соответствует ожидаемому")
